                                ...
                                <tr wire:key="{{ $activity->id }}">
                                    ...
                                    <td class="text-center" style="width: 250px;">
                                        @if(!empty($activity->image))
                                            <img src="{{ gallery_image($activity->image, \App\Constant\GalleryFileConstant::TYPE_ACTIVITY_IMAGE, \App\Constant\GalleryFileConstant::TYPE_COMPRESSED) }}"
                                    ...
                                    <td class="text-center">
                                        {!! \Str::limit($activity->excerpt, 50) !!}
                                    </td>
                                    <td class="text-center">
                                        {{ $activity->weight }}
                                    </td>
                                    <td class="text-center" style="width: 1%">
                                        <button wire:click="updateStatus({{ $activity->id }})" class="btn btn-icon btn-outline-{{ $activity->is_enabled ? 'success' : 'warning' }} btn-sm mr-2">
                                            <i class="{{ $activity->is_enabled ? 'flaticon2-check-mark' : 'fa fa-pause' }}"></i>
                                        </button>
                                    </td>
                                    <td class="text-center">
                                        <a href="{{ route('admin.activity.edit', ['id' => $activity->id]) }}" role="button" class="btn btn-sm btn-outline-brand btn-icon">
                                            <i class="flaticon-edit"></i>
                                        </a>

                                        @if($confirming === $activity->id)
                                            <button wire:click="deleteActivity({{ $activity->id }})" role="button"
                                        ...
                                        @else
                                            <button wire:click="confirmDelete({{ $activity->id }})" role="button"
